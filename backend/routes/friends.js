...
// remove a friend (optionally revoke access)
router.post('/remove', auth, async (req, res) => {
  try {
    ...
    if (revokeAccess) {
      // also remove friend and their children from access graph
      const accessGraphs = await AccessGraph.find({ 'authorizedUsers.user': req.user.id });

      for (const access of accessGraphs) {
        const isDirectChild = access.authorizedUsers.some(
          u => u.user.toString() === friendId && u.parent && u.parent.toString() === req.user.id
        );
